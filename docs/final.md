...
<iframe width="560" height="315" src="https://www.youtube.com/embed/2y5RfgqgHxk?si=MpD7GMA7V1KlG8Wv" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>  


## Project Summary  
The Stealth Escape project aims to develop a reinforcement learning (RL) agent capable of solving a maze within Minecraft via Project Malmo. The goal of the agent is to escape the maze while avoiding dangers such as lava pits and patrolling zombie guards. The agent is trained to make decisions based on its current state (position, light levels, and proximity to obstacles) and adapt its behavior to minimize the risk of falling into hazards or being caught by guards. By leveraging Q-learning and a reward-based system, the agent's actions are refined over time, improving its ability to achieve the mission's goal. The continuous learning aspect in RL hones in the pathfinding and decision-making techniques through trial and error as the agent learns to balance exploration with path exploitation to achieve its objective.

The task is challenging on several accounts. Since the environment is dynamic and partially observable-the agent cannot witness the maze as a whole and must respond to the moving guards and hazards in real time-it also has to manage competing objectives: finding the exit, avoiding lava, and remaining undetected by guards. Such environments make traditional rule-based approaches ineffective because they are not adaptable to unpredictable changes or lack the capability of learning from experiences. This is where reinforcement learning becomes significant. RL is when an agent learns through trial and error and improves strategies based on rewards and punishments. For example, it may be that an agent gets a reward for reaching the exit but is punished for falling into lava or being caught by the guard. Hence, the agent learns how to navigate the maze more efficiently over time.

Through this problem, the StealthEscape project can showcase what AI can do for difficult challenging real-life problems with varying uncertainties, dynamic obstacles, and adaptive decision-making. The skills acquired by the agent, such as hazard avoidance or navigation in stealthy ways, have widespread applicability beyond the gaming world-for example, in robotics and autonomous systems. Ultimately, the project would show the efficacy of RL in training agents for functionality in unpredictable and hazardous environments.  
